...
type heaps = heap list

type 'a comp = heaps -> 'a * heaps
...
let deref (l : ident) : value comp =
  fun h -> let rec pom (hs : heaps) : value = 
    match hs with
    | [] -> failwith "blad"
    | x::xs -> (match H.find_opt l x with
        | Some x -> x 
        | None -> pom xs )
  in (pom h, h)  
...
let assgn (l : ident) (v : value) : unit comp =
  fun h -> let rec pom (hs : heaps) : heaps = 
    match hs with
    | [] -> failwith "blad"
    | [x] -> [H.add l v x]
    | x::xs -> (match H.find_opt l x with
        | Some _ -> (H.add l v x)::xs  
        | None -> x::pom xs )
  in ((), pom h) 

let local (l : ident) (v : value) : unit comp =
  fun h -> let pom (hs : heaps) : heaps = 
    match hs with
    | [] -> failwith "blad"
    | x::xs -> (H.add l v x)::xs
  in ((), pom h) 
