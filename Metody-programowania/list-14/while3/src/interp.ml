...
let eval_op (op : bop) : value -> value -> value comp =
  ...
  | Sub  -> int_op Bigint.( - )
  | Mult -> int_op Bigint.( * )
  | Div  -> int_op Bigint.( / )  | And  -> bool_op (&&)
  | Or   -> bool_op (||)
  | Eq   -> (fun v1 v2 h -> (VBool (v1 = v2), h))
  | Neq  -> (fun v1 v2 h -> (VBool (v1 <> v2), h))
  | Leq  -> cmp_op (fun x y -> Bigint.compare x y <= 0)
  | Lt   -> cmp_op (fun x y -> Bigint.compare x y < 0)
  | Geq  -> cmp_op (fun x y -> Bigint.compare x y >= 0)
  | Gt   -> cmp_op (fun x y -> Bigint.compare x y > 0)
  ...
  | Print e ->
    ...
    (match r with
     | VInt n ->
         ...
         let prefix = if String.length s > 100 then String.sub s 0 100 else s in
         print_string prefix;
         if String.length s > 12 then print_string " ...";
         print_newline ();
         return ()
     | VBool b ->
