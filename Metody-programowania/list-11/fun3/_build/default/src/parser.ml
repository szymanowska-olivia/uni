...
let _menhir_action_30 =
  fun _endpos_body_ _startpos__1_ args body f ret_tp ->
...
# 68 "src/parser.mly"
        (
          let (x1, tp1), rest = List.hd args, List.tl args in
          let it =
            List.fold_right
              (fun (x, tp) acc -> node _loc (Fun(x, tp, acc)) )
              rest
              body
          in
        node _loc (Funrec(f, x1, tp1, ret_tp, it)) 
        )
# 617 "src/parser.ml"
     : (RawAst.expr))
...
include struct
  ...
  and _menhir_run_72 : type  ttv_stack. (((ttv_stack, _menhir_box_main) _menhir_cell1_FUNREC _menhir_cell0_IDENT, _menhir_box_main) _menhir_cell1_fun_args, _menhir_box_main) _menhir_cell1_type_simpl -> _ -> _ -> _ -> _ -> _ -> _menhir_box_main =
    fun _menhir_stack _menhir_lexbuf _menhir_lexer _endpos _v _tok ->
      let MenhirCell1_type_simpl (_menhir_stack, _, ret_tp) = _menhir_stack in
      ...
      let _v = _menhir_action_30 _endpos_body_ _startpos__1_ args body f ret_tp in
