...
let rec infer_type env (e : expr) : typ =
  ...
  | Var  x ->
    ...
    | Some tp -> tp
    | None    -> report_error e.pos ("Unbound variable" ^ x); TUnit
    end
  ...
  | App(e1, e2) ->
    ...
    | TArrow(tp2, tp1) ->
      ...
      tp1
    | _ -> report_error e.pos "Type error"; TUnit
    end
  | Fst e ->
...
and check_type env e tp =
  ...
  if tp = tp' then ()
    ...
    report_error e.pos ("Type error: expected" ^ (string_of_type tp) ^ "instead of" ^ (string_of_type tp'))
  end
let check_program e =
  errors := [];
