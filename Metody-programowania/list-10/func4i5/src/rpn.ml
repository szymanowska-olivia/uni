 ...
 *)
let rec eval (env : env) (s : int list) (p : prog) : int =
  match p, s with
  | [], [n] -> n
  | [], _   -> failwith "error!"
  | (PushInt n :: p'), _ -> eval env (n :: s) p'
  | (PushVar x :: p'), _ ->
      let v =
        try M.find x env
...
let stack_size (p : prog) =
  let rec it p maxi i =
  match p with
  | []   -> maxi
  | (PushInt _ :: p') -> 
    let i' = i + 1 in 
    let maxi' = if i' > maxi then i' else maxi in 
    it p' maxi' i'
  | (PushVar _ :: p') -> 
    let i' = i + 1 in 
    let maxi' = if i' > maxi then i' else maxi in 
    it p' maxi' i'
  | (Binop _ :: p') ->
    let i' = i - 1 in (*usuwa inty ale dodaje wynik*)
    it p' maxi i'
  in it p 0 0
