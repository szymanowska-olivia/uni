...
let geth : heap comp = fun h -> (h, h)

let seth (new_heap : heap) : unit comp = fun _ -> ((), new_heap)
...
let eval_op (op : bop) : value -> value -> heap -> value * heap =
  ...
  | Div  -> (fun v1 v2 h ->
    match v1, v2, h with 
    | VInt x, VInt y, h-> if y = 0 then raise DivByZeroException else (VInt ((/) x y), h)
    | _ -> failwith "type error")
  | And  -> bool_op ( && )
  | Or   -> bool_op ( || )
...
let rec eval_env (env : env) (e : expr) (h : heap) : value * heap =
  ...
  | While (w, t) ->
     let rec it h =
     let (v, h) = eval_env env w h in
    match v with
    | VBool true -> 
      (try
        let (_, h) = eval_env env t h in it h
      with 
        | BreakException -> (VUnit, h)  
        | ContinueException -> it h)
    | VBool false -> (VUnit, h) 
    | _ -> failwith "type error"
  in
  it h
