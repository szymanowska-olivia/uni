...
| Throw -> return VUnit  (* Zwróć VUnit jako monadę *)
| Try (e1, e2) ->
    let* res1 = eval_env env e1 in  (* używamy let* do bindowania wyniku eval_env e1 *)
    match res1 with
    | VUnit -> eval_env env e2  (* Jeśli wynik to VUnit, kontynuujemy z e2 *)
    | v -> return v  (* W przeciwnym razie, zwróć wartość v opakowaną w monadzie *)

let eval e = eval_env M.empty e

let interp (s : string) : value =
  Comp.run (
    let* res = eval (parse s) in
    match res with
    | VUnit -> failwith "unhandled exception"
    | v -> return v
  )
