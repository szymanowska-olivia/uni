...
module Comp : COMP = struct
  ...
  type 'a comp = heap -> ('a option * heap) (*model który nie zmienia heap przy none*)
...
let run (c : 'a comp) : 'a =
  ...
  | Some v, _ -> v
  | None, _ -> failwith "evaluation failed"
